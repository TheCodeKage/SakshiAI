 ...
 * 
 * Pipeline:
 * 1. Audio bytes → STT (Whisper) → transcript
 * 2. Transcript → LLM (Qwen) → structured JSON
 * 3. JSON → IncidentRecord
 * 
 * The LLM is instructed to:
 * - Extract key incident details (type, who, threats, witnesses)
 * - Detect pattern language ("again", "like before", "this time")
...
- Detect repeat language like "again", "like before", "this time" — set patternFlag to true if found.
...
{
  "incidentType": "string — e.g. Physical threat, Verbal threat, Coercive control, Physical contact",
  ...
  "summary": "string — 1-2 sentence dignified summary of what happened"
}
    """.trimIndent()
